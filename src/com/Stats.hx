...
class Stats extends Sprite {	
	...
	private function update(e : Event) {
		...
		if( timer - 1000 > ms_prev ) {
			...
			xml.fps = new flash.xml.XMLList( "FPS: " + fps + " / " + stage.frameRate ); 
			xml.mem = new flash.xml.XMLList("MEM: " + mem);
			xml.memMax = new flash.xml.XMLList("MAX: " + mem_max);			
		...
		xml.ms = new flash.xml.XMLList("MS: " + (timer - ms));
		ms = timer;
